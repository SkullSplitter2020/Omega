...
class JsonRAFifoQueue(object):
    ...
    def _lockFileIsStale(self):
        with open(self.lockPath, 'r') as f:
            if time.time() - float(f.read()) > self.timeout:
                return True
        return False

    def lock(self):
        if os.path.exists(self.lockPath):
            if self._lockFileIsStale():
                return self._createLockFile()  # Lock file is stale so recreate and return
...
class XBMCJsonRAFifoQueue(JsonRAFifoQueue):
    ...
    def _lockIsStale(self):
        ...
        if not last:
            return True
        if time.time() - float(last) > self.timeout:
            return True
        return False
    ...
    def lock(self):
        if self._lockProperty():
            if self._lockIsStale():
                return self._createLockProperty()  # Lock file is stale so recreate and return

            for x in (range(self.timeout * 10)):  # We timeout because all accesses should not block long so if we wait too long that means the lock is invalid
                if not self._lockProperty():
                    break
                time.sleep(0.1)
