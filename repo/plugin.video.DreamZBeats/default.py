...
addon_id        = 'plugin.video.DreamZBeats'
addon           = Addon(addon_id, sys.argv)
AddonTitle      = '[COLOR magenta]DreamZBeats[/COLOR]'
...
baseurl         = ( 'https://www.dropbox.com/s/c03iq579kgplr0g/DreamZBeats.xml?raw=true' )
...
messagetext     = ( 'https://www.dropbox.com/s/vdnjbz3zt11z35k/info.xml?raw=true') 
dialog          = xbmcgui.Dialog()
                                                               
def GetMenu():
     popup()
     url = baseurl
     addDir('[B][COLOR magenta]News & Updates[/COLOR][/B]',url,6,updatesicon,fanarts)
...
def popup():
        ...
        if len(message)>1:
                ...
                if compfile == message:pass
                        ...
                        showText('[B][I][COLOR magenta]DreamZBeats Version 1.0.7  Kodi 19 Matrix[/COLOR][/I][/B]', message)
...
def YOUTUBE_CHANNEL(url):
	...
	for video in videos:
		...
		addLink('[COLOR magenta]' + item["name"] + ' - on ' + item["date"] + '[/COLOR]',item["url"],4,item["iconimage"],fanart)
...
def PLAYLINK(name,url,iconimage):
    ...
    dp.create(AddonTitle,"[COLORmagenta]DreamZBeats[/COLOR]",'[COLOR magenta]OPENING LINK[/COLOR]','')   
    dp.update(0)
